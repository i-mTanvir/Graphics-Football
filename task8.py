...
def load_texture(filename):
    try:
        ...
        raise RuntimeError(
            "Pillow is required to load textures. Install it with: pip install pillow"
        ) from exc

    img  = image_module.open(filename).convert("RGBA")   # open PNG, force RGBA (includes transparency)
    img  = img.transpose(image_module.FLIP_TOP_BOTTOM)   # OpenGL reads pixels bottom-up so we flip
